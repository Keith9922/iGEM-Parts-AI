// background-firefox.js - Firefox兼容的后台服务脚本
...
browser.runtime.onInstalled.addListener(details => {
  ...
  if (details.reason === 'install') {
    // Firefox不支持notifications API，使用console.log
...
browser.browserAction.onClicked.addListener(async (tab) => {
  ...
  if (tab.url && tab.url.includes('parts.igem.org')) {
    ...
    if (sidePanelWindowId) {
      try {
        await browser.windows.update(sidePanelWindowId, {focused: true});
        return;
      } catch (error) {
        // 侧边栏可能已关闭，重置ID
    ...
    // 创建新的侧边栏
    try {
      const sidePanel = await browser.windows.create({
    ...
    // 不在iGEM页面时，显示提示
    console.log('请在iGEM Parts Registry页面使用此插件');
  }
});
...
browser.tabs.onActivated.addListener(async (activeInfo) => {
  if (sidePanelWindowId) {
    try {
      ...
      if (tab.url && tab.url.includes('parts.igem.org')) {
        // 确保侧边栏保持打开状态
        await browser.windows.update(sidePanelWindowId, {focused: false});
      }
    } catch (error) {
      // 侧边栏可能已关闭
      sidePanelWindowId = null;
